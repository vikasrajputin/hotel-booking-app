...
public class RoomService {
    ...
    public List<RoomDTO> getRoomsByStar(LocalDate dateIn, LocalDate dateOut, String stateLocation, int adults, int children, double minPrice, double maxPrice, int stars) {
        List<Room> rooms = roomRepository.findRoomsByStar(dateIn, dateOut, stateLocation, minPrice, maxPrice, stars);
        return rooms.stream().map(room -> {
            RoomDTO roomDTO = convertToDto(room);
            roomDTO.setAdjustedPrice(calculatePrice(room, adults, children));
            roomDTO.setPerNightRoom(calculatePerNightRoom(room, adults));
    ...
    public List<RoomDTO> getRoomsByUserRating(String stateLocation, LocalDate dateIn, LocalDate dateOut, int adults, int children, double minPrice, double maxPrice, String reviewTitle, double ratingNumber) {
        List<Room> rooms = roomRepository.findRoomsByUserRating( stateLocation, dateIn, dateOut, adults, children, minPrice, maxPrice, reviewTitle, ratingNumber);
        return rooms.stream().map(room -> {
            RoomDTO roomDTO = convertToDto(room);
            roomDTO.setAdjustedPrice(calculatePrice(room, adults, children));
            roomDTO.setPerNightRoom(calculatePerNightRoom(room, adults));
    ...
    private RoomDTO convertToDto(Room room) {
        ...
        roomDTO.setReviewTitle(room.getReviewTitle());
        return roomDTO;
    }
